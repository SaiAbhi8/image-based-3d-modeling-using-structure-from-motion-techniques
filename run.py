...
def run(scale, plot, in_files, outfile, num_images, feat, calibration_matrix):
    ...
    # [0, 0, 1, 0]
    # ], dtype=np.float64)
    zero_col = np.zeros((calibration_matrix.shape[0], 1))
    K_init=np.hstack((calibration_matrix, zero_col))
    K_init[:2] *= scale/100
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--in_files", type=str, default="data/inputs", help="Folder that images are stored in. Images are assumed to be sequentially named.")
